/**
 * Basic PDF Viewer - Simple version without store dependencies
 * Used to verify basic rendering works before adding complexity
 */
...
export function SimplePdfViewerBasic() {
  ...
  useEffect(() => {
    ...
    const loadPdf = async () => {
      try {
        console.log('[SimplePdfViewerBasic] Loading PDF:', activeDocument.url)
        ...
        console.log('[SimplePdfViewerBasic] PDF loaded, pages:', pdfDoc.numPages)
        setPdf(pdfDoc)
        setNumPages(pdfDoc.numPages)
        setIsLoading(false)
      } catch (err: any) {
        if (!mounted) return
        console.error('[SimplePdfViewerBasic] Error loading PDF:', err)
